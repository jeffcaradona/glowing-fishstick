...
2. **Green-build gate** — Run, in order: `npm run lint`, `npm run format` (check-only if available), `npm run test:all`. Capture failures verbatim. _Depends on step 1._
...
1. **If `npm install` produces a noisy `package-lock.json` diff** — recommend: stop, show the diff, ask before committing. _(Option A: commit lockfile churn / Option B: investigate first.)_ Recommend B.
