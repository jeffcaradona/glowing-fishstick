...
This document describes a lightweight, code-first workflow architecture designed for:
...
This pattern is especially useful when integrating message queues, background workers, or long-running processes.
...
### 2) Workflow runs in the background
...
app.post('/submit', async (req, res, next) => {
  try {
    ...
    res.status(202).json({ workflowId, status: 'accepted' });
  } catch (err) {
    next(err);
  }
});
...
- Async completion notifications via SSE
- Clean separation between HTTP, business logic, and async orchestration

It delivers many benefits of systems like Temporal or Durable Functions without introducing heavyweight infrastructure.
