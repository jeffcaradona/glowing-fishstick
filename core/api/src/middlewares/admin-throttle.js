 ...
 * for expensive API routes. Returns 429 when the request count for a given
 * route exceeds the configured threshold within a time window.
 *
 * WHY: Metrics endpoints expose process.memoryUsage() and runtime data
 * that, under burst traffic, can amplify CPU/I/O pressure. Throttling
 * prevents resource exhaustion without requiring external state (Redis).
 *
 ...
 *
 * VERIFY IF CHANGED: Integration tests for 429 responses on metrics routes,
 * and health endpoint availability tests (health must bypass throttle).
 */
